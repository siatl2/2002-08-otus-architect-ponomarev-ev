...
public class InitializeExceptionHandlerRepeatCommandTwoTimesAndWriteLog implements Initializer<ExceptionHandler> {
    @Override
    public void initialize(ExceptionHandler handler) {
        ...
        handler.add(
                ...
                (command, exception) -> {
                    QueueOfCommand queue = QueueOfCommandImpl.getInstance();
                    queue.push(new RepeatCallOriginalCommand(command));
                }
        );
        ...
        handler.add(
                new RepeatCallOriginalCommand(null),
                new IllegalArgumentException(),
                (command, exception) -> {
                    QueueOfCommand queue = QueueOfCommandImpl.getInstance();
